...
def test_download_input_format(input_format, output_format):
    ...
    if output_format == "files":
        ...
        assert(len([x for x in tarfile.open(image_folder_name+"/00000.tar").getnames() if x.endswith(".jpg")]) == expected_file_count)
...
def test_webdataset():
    ...
    assert(len(tarfile.open(image_folder_name+"/00000.tar").getnames()) == 10)
